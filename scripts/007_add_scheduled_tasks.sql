...
-- Create enum for task types (idempotent)
DO $$ BEGIN
...
-- Create enum for schedule frequency (idempotent)
DO $$ BEGIN
...
-- Create the scheduled_tasks table (idempotent)
...
CREATE INDEX IF NOT EXISTS idx_scheduled_tasks_next_run ON scheduled_tasks(next_run_at) WHERE is_enabled = true;
CREATE INDEX IF NOT EXISTS idx_scheduled_tasks_profile ON scheduled_tasks(profile_id);
CREATE INDEX IF NOT EXISTS idx_scheduled_tasks_type ON scheduled_tasks(task_type);

-- Create scheduled_task_runs table to track execution history (idempotent)
...
-- Create indexes for run history (idempotent)
CREATE INDEX IF NOT EXISTS idx_scheduled_task_runs_task ON scheduled_task_runs(scheduled_task_id);
CREATE INDEX IF NOT EXISTS idx_scheduled_task_runs_started ON scheduled_task_runs(started_at);
...
DROP TRIGGER IF EXISTS update_scheduled_tasks_updated_at_trigger ON scheduled_tasks;
CREATE TRIGGER update_scheduled_tasks_updated_at_trigger
BEFORE UPDATE ON scheduled_tasks
FOR EACH ROW
EXECUTE FUNCTION update_scheduled_tasks_updated_at();

-- Insert default scheduled tasks (disabled by default, idempotent)
DO $$
BEGIN
    -- Only insert if these tasks don't already exist
    IF NOT EXISTS (SELECT 1 FROM scheduled_tasks WHERE name = 'Nightly Trends Recomputation') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM scheduled_tasks WHERE name = 'Weekly Database Cleanup') THEN
        INSERT INTO scheduled_tasks (name, task_type, frequency, hour, minute, is_enabled, config) VALUES
