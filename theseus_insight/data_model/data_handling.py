    ...
    def get_cursor(self):
        ...
        try:
            ...
            try:
                ...
                # Raise a more critical error if extensions are essential
                error_message = (
                    f"Fatal: Could not load sqlite-vec extensions (vector0, vss0). Error: {e}. "
                    "Vector search functionality will be unavailable. "
                    "Please ensure sqlite-vec is correctly installed and accessible in the library path."
                )
                # Consider creating a custom exception class for this if desired.
                # For now, re-raising OperationalError with a more descriptive message,
                # or raising a new error that clearly indicates the feature loss.
                # Let's use ImportError as it often signals missing dependencies/components.
