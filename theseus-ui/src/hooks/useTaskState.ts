...
export interface TaskState {
  ...
  stage: string; // Deprecated, keeping for backward compatibility if needed, or alias to currentStep/message
  progress: number;
  message: string;
  error: string | null;
  result?: any;
      ...
      try {
                ...
                try {
                  ...
                  if (taskData && (taskData.status === 'processing' || taskData.status === 'pending')) {
                    ...
                    activeTask = {
                      ...
                      metadata: taskData.metadata || undefined
                    };
                  } else {
        ...
        if (activeTask) {
          ...
          setTaskState({
            ...
            currentStep: activeTask.current_step,
            ...
            metadata: activeTask.metadata || undefined,
          });
        } else {
  ...
  useEffect(() => {
    if (lastMessage && taskState.taskId) {
      ...
      setTaskState(prev => ({
        ...
        status: lastMessage.overallStatus,
        currentStep: lastMessage.currentStep || null,
        ...
        metadata: lastMessage.metadata
          ? { ...(prev.metadata || {}), ...lastMessage.metadata }
          : prev.metadata,
      }));
